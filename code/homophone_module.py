...
# Set the logging level to a level that hides INFO messages
...
def homophone_checker(
    input_string,
    fill_mask,
    homophones_list=homophones_list,
    score_threshold=0,
):
    ...
    target_homophones = [
        ...
        if list(set([word]).intersection(set(all_homophones)))
    ]
    ...
    if len(target_homophones) < 1:
        ...
        for target_homophone_tuple in target_homophones:
            ...
            # Uses the potentially corrected sentence to give the most accurate content possible
            # Unidirectional check
            input_string_list = total_sentence.split(" ")

            # Get all homophone options from homophones list
            homophone_options = [
                homophone
                for homophone in homophones_list
                if target_homophone in homophone
            ]
            ...
            # Get results
            try:
                results = fill_mask(masked_string, top_k=50)
            except RuntimeError:
            ...
            sorted_results = sorted(
                token_string_dict.items(), key=lambda x: x[1], reverse=True
            )

            # Find top homophone in results
            homophone_results = [
                result for result in sorted_results if result[0] in homophone_options[0]
            ]
            ...
            if top_result == target_homophone:
                ...
                if len(homophone_results) > 1:
                    ...
                    if score_diff > score_threshold:
                        error_idx.append(
                            input_string.split(" ").index(target_homophone)
                        )
                        error.append(target_homophone)
                        final_sentence.append(
                            input_string.replace(
                                target_homophone, homophone_results[0][0]
                            )
                        )
                        correct_word.append(homophone_results[0][0])
                        is_error.append(True)

                        total_sentence = total_sentence.replace(
                            target_homophone, homophone_results[0][0]
                        )
                ...
                # If the difference is less than the threshold, return the original sentence
                    ...
                    final_sentence.append(
                        input_string.replace(target_homophone, homophone_results[0][0])
                    )
                    correct_word.append(homophone_results[0][0])
                    error.append(target_homophone)
...
def homophone_runner(sentences):
    ...
    for i, sentence in enumerate(sentences):
        if i % 1000 == 0:
            print("PROCESSOR DONE WITH:", i)
        dfs.append(homophone_checker(sentence, fill_mask=fill_mask))
